...
- **Multiple parallel chats** (☰ top-left, Ctrl+N new, Ctrl+Tab cycle) — each chat is its own agent session with its own model, permission mode, context % and transcript; a busy chat keeps streaming in the background while you talk in another. The ☰ opens a full-panel conversation list (the compact-widget pattern from Intercom/Zendesk); background replies light it up (and badge the collapsed orb) until you look.
...
- **Browser form filling** — with the [companion extension](../extension/) loaded, Jarvis reads the page you armed (real field labels, not pixels) and proposes form values you approve field by field. Passwords, payment and hidden bot-trap fields are excluded structurally, and forms are never submitted.
